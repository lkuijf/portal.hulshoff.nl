    ...
    <div class="products">
        @if (isset($data['products']))
            @foreach($data['products'] as $product)
                ...
                @include('snippets.product', ['product_id' => $product['id'], 'product_code' => $product['artikelCode'], 'product_image' => 'https://picsum.photos/300/200', 'product_info' => '<p>' . $product['omschrijving'] . '</p>'])
            @endforeach
        @else
            @for ($x=0;$x<12;$x++)
                @include('snippets.product', ['product_id' => 999, 'product_code' => 'abc0001', 'product_image' => 'https://picsum.photos/300/200', 'product_info' => '<p>Categorie: WERKPLEK</p><p>Inrichtingsconcept: TAFEL</p><p>Leverancier: Gispen</p><p>Soort: VERGADERTAFEL</p>'])
            @endfor
        @endif
    </div>
    {{-- Paginering --}}
